...
class GitCraftCli {
	...
	static GitCraftConfig handleCliArgs(String[] args) {
		...
		if (cli_args_parsed.hasOption("ornithe-intermediary-generation")) {
			int generation = cli_args_parsed.'ornithe-intermediary-generation';
			config.ornitheIntermediaryGeneration = generation;
		}
		config.patchLvt = cli_args_parsed.'patch-lvt';
		...
		if (cli_args_parsed.hasOption("signatures")) {
			SignaturesFlavour signatures = cli_args_parsed.'signatures';
			config.usedSignatures = signatures;
		}
		if (cli_args_parsed.hasOption("nests")) {
			NestsFlavour nests = cli_args_parsed.'nests';
			config.usedNests = nests;
		}
