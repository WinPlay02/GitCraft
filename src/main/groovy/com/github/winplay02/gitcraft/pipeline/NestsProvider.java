...
public record NestsProvider(Step step, Config config) implements StepWorker {
	...
	public StepStatus run(Pipeline pipeline, Context context) throws Exception {
		...
		StepStatus mergedStatus = config.nestsFlavour().provide(mcVersion, MinecraftJar.MERGED, mappingFlavour);
		if (mergedStatus.hasRun()) {
			return mergedStatus;
		}
		StepStatus clientStatus = config.nestsFlavour().provide(mcVersion, MinecraftJar.CLIENT, mappingFlavour);
		StepStatus serverStatus = config.nestsFlavour().provide(mcVersion, MinecraftJar.SERVER, mappingFlavour);
