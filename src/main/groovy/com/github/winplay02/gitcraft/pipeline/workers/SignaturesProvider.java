...
public record SignaturesProvider(StepWorker.Config config) implements StepWorker<OrderedVersion, StepInput.Empty> {
	...
	public StepOutput<OrderedVersion> run(Pipeline<OrderedVersion> pipeline, Context<OrderedVersion> context, StepInput.Empty input, StepResults<OrderedVersion> results) throws Exception {
		StepStatus mergedStatus = config.signaturesFlavour().provide(context, MinecraftJar.MERGED);
		if (mergedStatus.hasRun()) {
			return StepOutput.ofEmptyResultSet(mergedStatus);
		}
		StepStatus clientStatus = config.signaturesFlavour().provide(context, MinecraftJar.CLIENT);
		StepStatus serverStatus = config.signaturesFlavour().provide(context, MinecraftJar.SERVER);
